...
int LightIntensity=53;																//设置光源电压53是6.4v；8是10.6V
...
//	ADC1_ChanConf.Channel=ch; 
	while(HAL_ADC_ConfigChannel(&hadc1, &ADC_ChanConf) != HAL_OK) {;}
...
{
	int Current_ad = Get_Adc(0);
	int Voltage_ad = Get_Adc(1);
...
void AD5245_Write(uint8_t data)
{
	...
	HAL_I2C_Mem_Write(&hi2c2, ADDR_AD5245_Write, 0, I2C_MEMADD_SIZE_8BIT,&data,1,0xff);
}
